...
adata = adata[adata.obs.celltype == "Fast_cycling", :]
adata.shape
...
# loom directory
loom_path = os.path.join("resources", "Fast_cycling_processed_input.loom")
loom_path_output = os.path.join("resources", "Fast_cycling_processed_output.loom")
...
outpath_adj = os.path.join(project_dir, "results/Fast_cycling_adjacencies.csv")
...
outpath_motifs = "results/Fast_cycling_motifs.csv"
...
if not os.path.exists(loom_path_output):
    !pyscenic aucell $loom_path "results/Fast_cycling_motifs.csv" --output {loom_path_output} --num_workers {num_workers}
...
# plot the heatmap (Extended Data Figure 5a):
...
plt.savefig("figures/Fast_cycling_correlation_heatmap.pdf", dpi=72, bbox_inches='tight')
plt.show()
...
# Regulon enrichment
# load the motifs
df_motifs = load_motifs("results/Fast_cycling_motifs.csv")
# derive regulons
regulons = derive_regulons(df_motifs)

with open("results/Fast_cycling_motifs.dat", "wb") as f: 
...
outpath = os.path.join("results", "Fast_cycling_regulons_aucell.csv")
auc_mtx[["MYC", "FOS", "MYBL2", "FOXM1"]].to_csv(outpath)

# plot them on the new embedding space (Extended Data Figure 4b):
sc.pl.embedding(adata, basis="X_umap_aucell", 
                ...
                save="_Fast_cycling_rss_regulon_activity.pdf")
...
plt.savefig("figures/Fast_cycling_top5_per_type.pdf", dpi=600, bbox_inches = "tight")
plt.show()