...
adata = adata[adata.obs.celltype == "G0_arrested", :]
adata.shape
...
# loom directory
loom_path = os.path.join("resources", "G0_arrested_processed_input.loom")
loom_path_output = os.path.join("resources", "G0_arrested_processed_output.loom")
...
outpath_adj = os.path.join(project_dir, "results/G0_arrested_adjacencies.csv")
...
outpath_motifs = "results/G0_arrested_motifs.csv"
...
if not os.path.exists(loom_path_output):
    !pyscenic aucell $loom_path "results/G0_arrested_motifs.csv" --output {loom_path_output} --num_workers {num_workers}
...
# plot the heatmap (Figure 4a-main):
...
plt.savefig("figures/G0_arrested_correlation_heatmap.pdf", dpi=72, bbox_inches='tight')
plt.show()
...
# plot the clustermap (Figure 4a-left):
g=sns.clustermap(mean_auc_by_type_top_n, figsize=[20, 4], cmap="Reds", xticklabels=False, yticklabels=True, square=True, col_cluster=False)
plt.savefig("figures/G0_arrested_mean_auc_by_clustermap_dark.pdf", format="pdf")

# Regulon enrichment
# load the motifs
df_motifs = load_motifs("results/G0_arrested_motifs.csv")
# derive regulons
regulons = derive_regulons(df_motifs)

with open("results/G0_arrested_motifs.dat", "wb") as f: 
...
# write regulons SPDEF, NR2F2, FOXA1, ATF3, RUNX3 and SOX9 AUCell scores to a csv file
outpath = os.path.join("results", "G0_arrested_regulons_aucell.csv")
auc_mtx[["SPDEF(+)", "NR2F2(+)", "FOXA1(+)", "ATF3(+)", "RUNX3(+)", "SOX9(+)"]].to_csv(outpath)

# plot them on the new embedding space (Figure 4b):
sc.pl.embedding(adata, basis="X_umap_aucell", 
                color=["Regulon(SPDEF)", "Regulon(ATF3)", "Regulon(FOXA1)", "Regulon(RUNX3)", "Regulon(STAT2)", "Regulon(NR2F2)", "Regulon(KLF2)", "Regulon(SOX9)", "type"], 
                title=["Regulon(SPDEF)", "Regulon(ATF3)", "Regulon(FOXA1)", "Regulon(RUNX3)", "Regulon(STAT2)", "Regulon(NR2F2)", "Regulon(KLF2)", "Regulon(SOX9)", "type"], 
                palette=TYPE_COLORS, cmap="Reds",
                save="_G0_arrested_rss_regulon_activity.pdf")

# Regulon specificity scores------------------------------------------------
rss = regulon_specificity_scores(auc_mtx, adata.obs.type)
...
plt.savefig("figures/G0_arrested_top5_per_type.pdf", dpi=600, bbox_inches = "tight")
plt.show()
...
# extract the regulons in the tf_names list from df_regulons
df_regulons = df_regulons[df_regulons.name.isin(tf_names)]

# Figure 4a (selected motifs):
